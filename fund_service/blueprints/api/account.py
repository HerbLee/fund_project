...
async def register(request):
    ...
    if mobile is None or password is None:
        raise MissingArgumentException('mobile or password is None')
    if not check_utils.judge_phone(mobile):
        raise BadParameterException('mobile error')

    user_service = UserService(request)
    if await user_service.existence_mobile(mobile):
        return response_json(code=ResponseCode.FAIL, message="mobile existence")

    user = await user_service.create(mobile=mobile, password=password)
